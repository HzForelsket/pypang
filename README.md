...
- 单文件超过默认分卷阈值 `2GB` 时，会在远端自动创建一个 `原文件名.parts` 目录，再按当前账号档位允许的范围内以 `2GB` 为默认分卷大小顺序上传
- 多卷上传默认会并发发送 `2` 个卷；分卷预计算仍保持单线程顺序进行
...
- 单卷大小默认 `2GB`；同时仍受百度网盘开放平台单文件上限约束：普通用户 `4GB`、普通会员 `10GB`、超级会员 `20GB`
